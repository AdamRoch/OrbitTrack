...
|---|---|---|---|
| `GET` | `/api/issues` | — | Filters: `?status=`, `?priority=`, `?label=` (by name). |
| `GET` | `/api/issues/frontier` | — | What's grabbable right now. Ordered by priority desc, then created asc. |
| `GET` | `/api/issues/:id` | — | Full detail. `404` if not found. |
| `POST` | `/api/issues` | `{ title, description?, status?, priority?, labelNames?[] }` | `201` on success, `400` on bad input. |
...
| `PUT` | `/api/issues/:id/labels` | `{ labelNames: string[] }` | Full replacement. |
| `GET` | `/api/issues/:id/blockers` | — | Issues blocking this one. |
| `POST` | `/api/issues/:id/blockers` | `{ blockerId: number\|string }` | Creates edge "A blocks B". `400` on self-edge/cycle. |
...
| Method | Path | Body | Notes |
|---|---|---|---|
| `GET` | `/api/questions` | — | Open questions across issues. Filters: `?status=open` (default), `?label=` (a QA-agent "track"). Each entry embeds the full `Issue`. |
