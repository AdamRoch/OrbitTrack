...
/**
 * GET /api/issues/:id/questions?project=KEY — the issue's Q&A history, oldest
 * first. (This is also embedded in `GET /api/issues/:id` as `questions[]`.)
 */
...
/**
 * POST /api/issues/:id/questions?project=KEY  { question }
...
export async function POST(req: Request, ctx: RouteContext) {
  try {
    ...
    const result = addQuestion(db, project, id, question);
    if (!result.ok) {
